...
def make_window2():
    layout = [[sg.Text('HEADER')],
              ...
              [sg.Text('Compound:'),
               sg.Text(' ', text_color=('black'), background_color=(
                   'gray70'), size=(10, 1), key='-COMPOUND_TXT-'),
...
Compound_Data = pd.DataFrame([['', 0], ['', 0], ['', 0], ['', 0], ['', 0]], columns=[
                             'Compound', 'Compound_mass'])
...
while True:
        ...
        if values['-ELEMENT_LIST-']:
            if values['-QUANTITY-'] != []:
                if values['-QUANTITY-'][0] > 1:
                    ...
                    Atom_index = Element_Data[Element_Data['Element'] ==
                                              values['-ELEMENT_LIST-'][0]].index.tolist()[0]
                    Element_mass_added = Element_Data.loc[Atom_index,
                                                          'Atomic_Mass']*values['-QUANTITY-'][0]
                    Compound_Data.loc[Compound_index, 'Compound_mass'] = Compound_Data.loc[Compound_index,
                                                                                           'Compound_mass'] + Element_mass_added
    ...
    if window == window1 and (event in '-CALC-'):
        ...
        stry = values['-MOL_IN1-']+values['-MOL_IN2-']+values['-MOL_IN3-'] + \
            values['-MOL_IN4-']+values['-MOL_IN5-']+values['-GRAMS-']
        ...
        if only_digits == True:
            Total_sum_mol = fsum([float(values['-MOL_IN1-']), float(values['-MOL_IN2-']),
                                  float(values['-MOL_IN3-']), float(values['-MOL_IN4-']), float(values['-MOL_IN5-'])])
            if Total_sum_mol == 100.0:

                Compound_Data.loc[0, 'mass_x_fracmol'] = Compound_Data.loc[0,
                                                                           'Compound_mass']*float(values['-MOL_IN1-'])/100
                Compound_Data.loc[1, 'mass_x_fracmol'] = Compound_Data.loc[1,
                                                                           'Compound_mass']*float(values['-MOL_IN2-'])/100
                Compound_Data.loc[2, 'mass_x_fracmol'] = Compound_Data.loc[2,
                                                                           'Compound_mass']*float(values['-MOL_IN3-'])/100
                Compound_Data.loc[3, 'mass_x_fracmol'] = Compound_Data.loc[3,
                                                                           'Compound_mass']*float(values['-MOL_IN4-'])/100
                Compound_Data.loc[4, 'mass_x_fracmol'] = Compound_Data.loc[4,
                                                                           'Compound_mass']*float(values['-MOL_IN5-'])/100
                ...
                for gui in Compound_Data.index.tolist():
                    Compound_Data.loc[gui, 'mass_needed'] = Compound_Data.loc[gui,
                                                                              'fraction'] * float(values['-GRAMS-'])
